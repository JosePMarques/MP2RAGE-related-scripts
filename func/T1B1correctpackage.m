function [B1temp, T1temp, MP2RAGEcorrected] = T1B1correctpackage(Sa2RAGEimg, B1img, Sa2RAGE, MP2RAGEimg, T1img, MP2RAGE, brain, invEFF)
% Usage:
%
% [B1corr, T1corr, MP2RAGEcorr] = T1B1correctpackage(Sa2RAGEimg, B1, Sa2RAGE, MP2RAGEimg, T1img, MP2RAGE, brain, invEFF)
%
% Sa2RAGEimg (and B1) and MP2RAGEimg (and T1img) are the nii structures resulting from loading the
...
% You only have to load either the MP2RAGEimg or the T1img (I usually use the MP2RAGEimg)
...
hold off
for B1 = 0.6:0.2:1.4
    [MP2RAGEamp, T1vector] = MP2RAGE_lookuptable(2, MP2RAGE.TR, MP2RAGE.TIs, B1*MP2RAGE.FlipDegrees, MP2RAGE.NZslices, MP2RAGE.TRFLASH, 'normal');
...
if showimages==1
    ...
    subplot(122)
        imagesc(Sa2RAGE_vector, T1_vector, B1matrix)
        colorbar
        axis tight
